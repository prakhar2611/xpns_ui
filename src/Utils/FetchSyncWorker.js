...
export function SyncWorker (token,from,to,label)  {
        ...
        const URL = `http://192.168.1.5:9005/SyncMail?label=${label}&to=${to}&from=${from}`;
...
export function FetchWorker(token,from,to,label,callback, errorcallback){
    ...
    const URL = `http://192.168.1.5:9005/expense/api/v1/getExpense?from=${from}&to=${to}&label=${label}`;
    return axios(URL, {
    method: 'GET',
    headers: {
      'content-type': 'application/json', 
...
export function UpdateVPAMapping(p,token){
  ...
  return axios.post('http://192.168.1.5:9005/expense/api/v1/UpdateVpaMapping', JSON.stringify(p),{
      headers: {
          'Content-Type': 'application/json',
          'token' :  token
       },
...
export function FetchGroupedVPA(limit,offset,token){
  ...
  console.log("Request payload for fetching the data : ", `http://192.168.1.5:9005/expense/api/v1/getXpnsByVpa` )


       return      axios.get(`http://192.168.1.5:9005/expense/api/v1/getXpnsByVpa?label=HDFC&limit=${limit}&offset=${offset}`,{
            headers: {           
                'Content-Type': 'application/json',
                'token' :  token
            },        
